...
public class RaspiCoinsSendCommandContainer {
    ...
    public static LiteralCommandNode<CommandSourceStack> command() {
        return Commands.literal("sendcoins")
                .then(Commands.argument("player", StringArgumentType.string()).suggests((context, builder) -> RaspiSuggestions.suggestOfflinePlayer(builder)).then(Commands.argument("value", IntegerArgumentType.integer(1, 999999999)).executes(RaspiCoinsSendCommandContainer::exc))).build();
    }
