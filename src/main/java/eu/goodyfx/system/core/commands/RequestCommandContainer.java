...
public class RequestCommandContainer {
    ...
    private static int executeConfirm(CommandContext<CommandSourceStack> context) {
        ...
        Raspi.players().withOnlinePlayer(dummy, player -> {
            ...
            if (targetDummy != null) {
                Raspi.players().withOnlinePlayer(targetDummy, raspiPlayer -> {
    ...
    private static int executeRequestDeny(final CommandContext<CommandSourceStack> context) {
        ...
        Raspi.players().withOnlinePlayer(dummy, player -> {
            ...
            if (target.isOnline()) {
                Raspi.players().withOnlinePlayer(Objects.requireNonNull(target.getPlayer()), targetP -> {
                    playerDeny(reason, dummy, targetP.userData());

                    allowed.set(String.format(allowed.get(), targetP.getColorName()));
    ...
    private static int executeRequestAccept(final CommandContext<CommandSourceStack> context) {
        ...
        Raspi.players().withOnlinePlayer(dummy, player -> {
            ...
            if (target.isOnline()) {
                ...
                Raspi.players().withOnlinePlayer(Objects.requireNonNull(target.getPlayer()), targetPlayer -> {
                    playerAllow(simpleDateFormat, player.getPlayer(), targetPlayer.userData());
