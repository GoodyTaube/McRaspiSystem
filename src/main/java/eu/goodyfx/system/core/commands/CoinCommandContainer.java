...
public class CoinCommandContainer {
    ...
    private final static String SUCCESS_COINS_SEND = "Du hast erfolgreich eine Transaction gestartet.";
    private final static String SUCCESS_COINS_SEND_HOVER = "<hover:show_text:'%s an %s --> %s'>";

    public static LiteralCommandNode<CommandSourceStack> command() {
        return Commands.literal("coins").executes(CoinCommandContainer::coinsCommand).then(Commands.literal("send")
                .then(Commands.argument("player", StringArgumentType.string())
                        .suggests(((context, builder) -> RaspiSuggestions.suggestOfflinePlayer(builder))))
                .then(Commands.argument("amount", IntegerArgumentType.integer(1, 9999)).executes(CoinCommandContainer::coinsSendCommand))
        ).build();
    }
    ...
    private static int coinsSendCommand(CommandContext<CommandSourceStack> context) {
        ...
        if (!(amount < playerCoins)) {
            raspiPlayer.sendMessage(String.format(FAIL_COINS_AMOUNT, playerCoins, amount));
            return 1;
        }
        ...
        raspiPlayer.sendMessage(hover + SUCCESS_COINS_SEND, true);
