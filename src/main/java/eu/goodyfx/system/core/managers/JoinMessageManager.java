...
public class JoinMessageManager {
    ...
    public String get(Player player) {
        ...
        Raspi.players().withOnlinePlayer(player, raspiPlayer -> {
                ...
                for (String groups : Objects.requireNonNull(manager.config().getConfigurationSection("join.groups")).getKeys(false)) {
                    ...
                    if (Boolean.TRUE.equals(manager.contains(groupPath + groups + ".container"))) {
                        ...
                        if (userContainer.contains(name)) {
                            ret.set(Objects.requireNonNull(manager.config().getString(groupPath + groups + ".message")).replace("{player}", raspiPlayer.getColorName()));
                            return;
                        }
                    }
                }
            ...
            ret.set(Objects.requireNonNull(manager.config().getString("join.default-welcome-message")).replace("{player}", raspiPlayer.getColorName()));
        });
        return ret.get();
    }
