...
public class TablistAnimator extends BukkitRunnable {
    ...
    private static final String WAITING_PLAYERS = "<gray>Wartende Spieler: 0 / %s";

    private int index;

    public TablistAnimator() {
        runTaskTimerAsynchronously(JavaPlugin.getPlugin(McRaspiSystem.class), 0, 7L);
        Component waitingPlayers = MiniMessage.miniMessage().deserialize(String.format(WAITING_PLAYERS, Bukkit.getOnlinePlayers().size()));
    }

    @Override
    public void run() {
        Component animated = getAnimatedTitle();
        Component header = Component.empty().append(animated);
        sendPlayer(header);
        index++;
        if (index >= tTA.length()) {
            index = 0;
    ...
    private Component getAnimatedTitle() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            full = full.append(Component.text(String.valueOf(cha)).color(net.kyori.adventure.text.format.TextColor.color(r, g, b)));
        }
