...
public class ResetSubCommand implements RaspiSubCommand {

    @Override
    public ArgumentBuilder<CommandSourceStack, ?> build() {
        return Commands.literal("reset").then(Commands.argument("id", IntegerArgumentType.integer(1, 9999999)).executes(this::removeExecute));
    }
    ...
    private int removeExecute(CommandContext<CommandSourceStack> context) {
        ...
        raspiPlayer.sendMessage(String.format("<green>Du hast %s resettet. Die ID ist nun für einen neuen Spieler frei!", id), true);
